...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        # это означает подтверждение того, что присутствуют имя пользователя и пароль
        # и то, что эта комбинация соответствует одному из пользователей.
        username = self.initial_data.get('username')
        email = data.get('email', None)
        password = data.get('password', None)

        # Вызвать исключение, если не предоставлено имя пользователя.
        if username is None:
        ...
        # предоставленные имя и пароль соответствуют какому-то пользователю в
        # нашей базе данных.
        user = authenticate(username=username, password=password)

        # Если пользователь с данными именем/паролем не найден, то authenticate
